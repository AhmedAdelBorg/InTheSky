...
export default defineComponent({
  ...
  async mounted() {
    const result = await axios.get(
      "https://intheskyserver.onrender.com/dataPigeon/" + this.$route.params.id
    );
    if (result.status == 200) {
      this.dataPigeon = result.data;
      this.extraLists = result.data.extra;
      let textExtra = this.extraLists.join("\n");
      this.dataPigeon.extra = textExtra;
    }
    const request = await axios.get(
      "https://intheskyserver.onrender.com/dataPigeon"
    );

    let dataLists = request.data;
